...
void aes_key_schedule_256(const uint8_t *key, uint8_t *roundkeys) {
    ...
    // Copy the first 32 bytes (master key) into round keys
    for (i = 0; i < 32; ++i) {
        roundkeys[i] = key[i];
    }

    for (i = 32; i < AES_ROUND_KEY_SIZE_256; i += 4) {
        // Get the previous 4 bytes
        for (j = 0; j < 4; ++j) {
        ...
        // Every 8th word
        if (i % 32 == 0) {
            // Rotate, SubBytes, Rcon
            uint8_t t = temp[0];
            temp[0] = SBOX[temp[1]] ^ RC[(i / 32) - 1];
            temp[1] = SBOX[temp[2]];
            temp[2] = SBOX[temp[3]];
            temp[3] = SBOX[t];
        } else if (i % 32 == 16) {
        ...
        // XOR with previous 32-byte block
        for (j = 0; j < 4; ++j) {
            roundkeys[i + j] = roundkeys[i - 32 + j] ^ temp[j];
        }
    }
}
